    ...
    getAllItems("team_members", function(err, team_members) {
        getAllItems("sprints", function(err, allSprints) {
            ...
            for (let i = 0; i < allSprints["Count"]; i++) {
                ...
                if (allSprints["Items"][i]["sprint_id"] == sprint_id.toString()) {
                    allQueuedTasks = allSprints["Items"][i]["taskIDS"]
                }
            }
                ...
                if (err) console.log(err);
                    ...
                    if (typeof(allTasks) !== "undefined") {
                        ...
                        allTasks["Items"].forEach(function(task) {
                            ...
                            if (allQueuedTasks.includes(parseInt(task["task_id"]))) {
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button title="Delete task, removing it from product backlog" type="button" class="btn btn-danger"  data-dismiss="modal" onclick="removeTaskFromSprint(${task.task_id})">Delete Task</button>
                ...
                'false',
                'false',
                document.getElementById('descriptionBox${task.task_id}').value,'${sprint_id}')" title="Update task, save changes." data-dismiss="modal"  >Save changes</button>
            </div>
        </div>
        </div>
    </div>`;
                ...
                if (err) console.log(err);
                    ...
                    if (filter == "All") {
                        ...
                        allTasks["Items"].forEach(function(task) {
                            ...
                            if (task.deleted == false && task.status == "Not Started") {
                                //if the entire row has been filled with cards
                                if (cardRowCount === MAXCARDPERROW) {
                                    //reset card count
                                    cardRowCount = 0;
                        ...
                        allTasks["Items"].forEach(function(task) {
                            //if task matches filter, add to html
                            if (task.tags == filter && task.deleted == false && task.status == "Not Started") {
                                //if the entire row has been filled with cards
                                if (cardRowCount === MAXCARDPERROW) {
                                    //reset card count
                                    cardRowCount = 0;
...
function removeTaskFromSprint(task_id) {
    if (confirm("Are you sure you want to remove task from sprint?") == true) {
        getAllItems("sprints", function(err, allSprints) {
            allSprints["Items"].forEach(function(sprint) {
                if (sprint["sprint_id"] == sprint_id.toString()) {
                    newTaskID = sprint["taskIDS"].splice(sprint["taskIDS"].indexOf(task_id), 1);
